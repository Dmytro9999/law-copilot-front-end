...
export default function ForgotPasswordPage() {
	...
	async function onSubmit(e: React.FormEvent) {
		...
		try {
			...
			const msg = err?.data?.message || err?.message || t('forgot.toast.defaultError')
			toast({ title: t('forgot.toast.errorTitle'), description: msg, variant: 'destructive' })
		}
	}

	const handleSuccessOpenChange = (open: boolean) => {
		setOpenSuccess(open)
		if (!open) router.replace(`/${lang}/login`)
	}
	...
	return (
		<>
			...
			<Dialog open={openSuccess} onOpenChange={handleSuccessOpenChange}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>{t('auth.forgot.dialog.title')}</DialogTitle>
						<DialogDescription>{t('auth.forgot.dialog.desc')}</DialogDescription>
					</DialogHeader>
					<div className='flex justify-end'>
						<Button onClick={() => handleSuccessOpenChange(false)}>
							{t('auth.forgot.dialog.ok')}
						</Button>
					</div>
				</DialogContent>
			</Dialog>
